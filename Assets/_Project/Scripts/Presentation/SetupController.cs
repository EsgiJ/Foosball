...
namespace Foosball.Presentation
{
    ...
    #region Lifecycle
        public void Init(AudioManager audioManager, GameStateManager gameStateManager)
        {
        ...
        void OnEnable()
        {
            ...
            if (m_GameStateManager.CurrentState == GameState.Setup)
                EnterSetup();
        }
        ...
        {
            if (m_GameStateManager == null ||
                m_GameStateManager.CurrentState != GameState.Setup) return;
    ...
    #region Setup flow
        ...
        private void CycleFormation(int side, int dir)
        {
            ...
            m_AudioManager?.PlayMenuButtonClick();
            RefreshSide(side);
        }
        ...
        private void ToggleReady(int side)
        {
            var p = GetPlayerOnSide(side);
            p.ready = !p.ready;
            m_AudioManager?.PlayToggleReady();
            RefreshSide(side);
        ...
        private void ApplyAndStart()
        {
            ...
            m_GameStateManager.StartCountdown();
        }
